...
class EventBus extends EventEmitter {
  constructor() {
    ...
    // Add default error listener to prevent Node.js from crashing on unhandled 'error' events
    this.on('error', (err) => {
      // Intentionally swallow or just log silently if no one is listening
      // console.warn('[EventBus] Error event emitted:', err.message || err);
    });
  }
