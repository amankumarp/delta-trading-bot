...
async function backfillMissing1m(symbol, from, to) {
  ...
  const totalChunks = Math.ceil((targetTo - from) / CHUNK_SIZE);
  let chunkCount = 0;

  while (currentEnd > from) {
      chunkCount++;
      let currentStart = Math.max(from, currentEnd - CHUNK_SIZE);
      let chunksRemaining = Math.max(0, totalChunks - chunkCount);
      ...
      try {
          ...
          // Emit progress event
          EventBus.emit('data_backfilled', { 
              symbol, 
              chunkStart: new Date(currentStart * 1000).toLocaleDateString(), 
              chunkEnd: new Date(currentEnd * 1000).toLocaleDateString(),
              chunksRemaining 
          });
