...
function calculateKlusterSupertrend(){
    // Import technical indicators library
const { ATR, EMA } = require('technicalindicators');

// Settings (equivalent to Pine Script inputs)
const length = 10;          // ATR Length
const minMult = 1;          // Factor Range Min
const maxMult = 5;          // Factor Range Max
const step = 0.5;           // Step
const perfAlpha = 10;       // Performance Memory
const fromCluster = 'Best'; // From Cluster: 'Best', 'Average', 'Worst'
const maxIter = 1000;       // Maximum Iteration Steps
const maxData = 10000;      // Historical Bars Calculation

// Validate settings
if (minMult > maxMult) {
    throw new Error('Minimum factor is greater than maximum factor in the range');
}

// Sample price data (replace with your OHLCV data)
const bars = [
    // Example format: { open: 100, high: 102, low: 99, close: 101, volume: 1000 },
    // Add your historical data here
];

// Preprocess data: Calculate hl2
bars.forEach(bar => {
...
const atr = Array(length - 1).fill(NaN).concat(atrValues);

// Supertrend class definition
class Supertrend {
    constructor(factor) {
        this.upper = NaN;
        this.lower = NaN;
...
const holder = [];
for (let i = 0; i <= Math.floor((maxMult - minMult) / step); i++) {
    const factor = minMult + i * step;
    holder.push(new Supertrend(factor));
}

// Calculate EMA denominator for performance index
const absDiff = bars.map((bar, i) => i > 0 ? Math.abs(bar.close - bars[i - 1].close) : 0);
const denEMA = EMA.calculate({ period: Math.floor(perfAlpha), values: absDiff });
...
for (let i = 1; i < bars.length; i++) {
    const atrValue = atr[i];
    if (isNaN(atrValue)) {
        results.push({ ts: NaN, perf_ama: NaN, os: NaN });
        continue;
    }

    // Update Supertrend calculations for all factors
    holder.forEach(spt => {
        const up = bars[i].hl2 + atrValue * spt.factor;
        const dn = bars[i].hl2 - atrValue * spt.factor;

        if (isNaN(spt.upper)) {
            spt.upper = up;
            spt.lower = dn;
        } else {
            spt.upper = bars[i - 1].close < spt.upper ? Math.min(up, spt.upper) : up;
            spt.lower = bars[i - 1].close > spt.lower ? Math.max(dn, spt.lower) : dn;
        }

        if (bars[i].close > spt.upper) {
            spt.trend = 1;
        } else if (bars[i].close < spt.lower) {
            spt.trend = 0;
        }

        spt.output = spt.trend === 1 ? spt.lower : spt.upper;

        const diff = isNaN(spt.output) ? 0 : Math.sign(bars[i - 1].close - spt.output);
        const profit = (bars[i].close - bars[i - 1].close) * diff;
        spt.perf += (2 / (perfAlpha + 1)) * (profit - spt.perf);
    });

    let target_factor = results[i - 1]?.target_factor || NaN;
    let perf_idx = 0;

    // Perform K-means clustering if within maxData bars from the end
    if (i >= Math.max(0, bars.length - 1 - maxData)) {
        const data = holder.map(spt => spt.perf);
        const factor_array = holder.map(spt => spt.factor);

        // Initialize centroids using approximate quartiles
        const sortedData = [...data].sort((a, b) => a - b);
        const p25 = sortedData[Math.floor(0.25 * sortedData.length)];
        const p50 = sortedData[Math.floor(0.5 * sortedData.length)];
        const p75 = sortedData[Math.floor(0.75 * sortedData.length)];
        let centroids = [p25, p50, p75];

        // K-means clustering
        let perfClusters, factorClusters;
        for (let iter = 0; iter < maxIter; iter++) {
            ...
            data.forEach((value, idx) => {
                ...
                perfClusters[clusterIdx].push(value);
                factorClusters[clusterIdx].push(factor_array[idx]);
            });

            // Update centroids
            const newCentroids = perfClusters.map(cluster => 
                cluster.length > 0 ? cluster.reduce((a, b) => a + b, 0) / cluster.length : centroids[perfClusters.indexOf(cluster)]
            );

            // Check convergence
            if (newCentroids.every((c, idx) => c === centroids[idx])) {
                break;
            }
        ...
        const clusterIndex = fromCluster === 'Best' ? 2 : fromCluster === 'Average' ? 1 : 0;
        const selectedFactors = factorClusters[clusterIndex];
        const selectedPerf = perfClusters[clusterIndex];

        if (selectedFactors.length > 0) {
            target_factor = selectedFactors.reduce((a, b) => a + b, 0) / selectedFactors.length;
            const avgPerf = selectedPerf.reduce((a, b) => a + b, 0) / selectedPerf.length;
            perf_idx = Math.max(avgPerf, 0) / denEMA[i];
        }
    }

    // Calculate final SuperTrend with target_factor
    const up = bars[i].hl2 + atrValue * target_factor;
    const dn = bars[i].hl2 - atrValue * target_factor;

    let upper = results[i - 1]?.upper || bars[i].hl2;
    let lower = results[i - 1]?.lower || bars[i].hl2;
    let os = results[i - 1]?.os || 0;

    upper = bars[i - 1].close < upper ? Math.min(up, upper) : up;
    lower = bars[i - 1].close > lower ? Math.max(dn, lower) : dn;
    os = bars[i].close > upper ? 1 : bars[i].close < lower ? 0 : os;
    const ts = os ? lower : upper;

    // Calculate adaptive moving average (perf_ama)
    let perf_ama = results[i - 1]?.perf_ama;
    if (isNaN(perf_ama) && !isNaN(ts)) {
        perf_ama = ts;
    } else if (!isNaN(perf_ama)) {
        perf_ama += perf_idx * (ts - perf_ama);
    }

    // Store results for this bar
    results.push({ ts, perf_ama, os, target_factor, upper, lower });
}

// Output results (e.g., last bar's data)
console.log('Last bar results:', results[results.length - 1]);

// Optional: Output all results as JSON
